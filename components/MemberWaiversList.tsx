// Shared component for the member's waiver list. Used at the top-level
// /member/waivers (all the gyms a member belongs to) and inside the
// per-gym dashboard at /m/[gymId]/waivers (just this gym).
...
type Waiver = { id: string; gym_id: string; title: string; content: string };
...
export function MemberWaiversList({
  gymIdFilter,
  showHeader = true,
}: {
  gymIdFilter?: string | null;
  showHeader?: boolean;
}) {
  ...
  const load = useCallback(async () => {
    ...
    if (gymIdFilter) {
      ...
      const { data: memberships } = await supabase
        ...
        .eq('member_id', myId)
        .eq('status', 'active');
      gymIds = Array.from(new Set((memberships ?? []).map((m: any) => m.gym_id)));
    }
    if (gymIds.length === 0) { setWaivers([]); return; }
    ...
    (gyms ?? []).forEach((g: any) => { names[g.id] = g.name; });
    setGymNames(names);
    setWaivers((w as Waiver[]) ?? []);
    const sigMap: Record<string, string> = {};
    ((sigs as Sig[] | null) ?? []).forEach((s) => { sigMap[s.waiver_id] = s.signed_at; });
    setSigned(sigMap);
  }, [myId, gymIdFilter]);
  ...
  async function submit(w: Waiver) {
    if (!form || !myId) return;
    setErr(null);
    if (!form.participant_name.trim()) { setErr('Enter the participant name.'); return; }
    if (!form.typed_signature.trim()) { setErr('Type your signature.'); return; }
    if (!form.unauthorized_initials.trim()) { setErr('Add your initials to the unauthorized-entry box.'); return; }
    if (!form.agreed) { setErr('You must agree to the waiver to sign it.'); return; }
    setSaving(true);
    const { error } = await supabase.from('gym_waiver_signatures').insert({
      waiver_id: w.id, gym_id: w.gym_id, member_id: myId,
  ...
  if (waivers === null) {
    return <View style={styles.center}><ActivityIndicator color={theme.colors.charcoal} /></View>;
  }
      ...
      {showHeader ? (
        <View>
          ...
          <Text style={styles.sub}>
            {gymIdFilter
              ? "Review and sign this gym's waivers."
              : "Review and sign your gyms' waivers."}
          </Text>
        </View>
      ) : null}
            ...
            <View key={w.id} style={styles.card}>
              <View style={styles.cardHead}>
                <View style={{ flex: 1 }}>
                  ...
                  {!gymIdFilter ? <Text style={styles.cardGym}>{gymNames[w.gym_id] ?? 'Gym'}</Text> : null}
                </View>
                {isSigned ? (
...
function Field({
  ...
  label: string; value: string; onChange: (v: string) => void; flex?: boolean;
}) {
  return (
    <View style={[styles.field, flex && { flex: 1, minWidth: 150 }]}>
      <Text style={styles.label}>{label}</Text>
      <TextInput value={value} onChangeText={onChange} placeholderTextColor="#94a3b8" style={styles.input} />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 12, borderWidth: 1, borderColor: theme.colors.border, backgroundColor: '#fff', padding: 14, gap: 12 },
  ...
  signedBadge: { paddingHorizontal: 10, paddingVertical: 5, borderRadius: 999, backgroundColor: '#ecfdf5' },
  ...
  ghostBtn: { paddingHorizontal: 14, paddingVertical: 9, borderRadius: 10, borderWidth: 1, borderColor: theme.colors.border },
  ...
  waiverBox: { maxHeight: 320, overflow: 'scroll', borderWidth: 1, borderColor: theme.colors.border, borderRadius: 10, padding: 14, backgroundColor: '#fbfcfe' },
  section: { fontSize: 14, fontWeight: '800', color: theme.colors.charcoal, marginTop: 6 },
  row: { flexDirection: 'row', gap: 10, flexWrap: 'wrap' },
  field: { gap: 4 },
  label: { fontSize: 13, fontWeight: '700', color: theme.colors.charcoal },
  input: { borderWidth: 1, borderColor: theme.colors.border, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 10, fontSize: 14, backgroundColor: '#fff', color: theme.colors.charcoal },
  initialsBox: { gap: 8, padding: 12, borderRadius: 10, borderWidth: 1, borderColor: '#fde68a', backgroundColor: '#fffbeb' },
  initialsNotice: { fontSize: 13, color: '#92400e', lineHeight: 19 },
  agreeRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 10, marginTop: 4 },
  checkBox: { width: 22, height: 22, borderRadius: 5, borderWidth: 1, borderColor: theme.colors.border, alignItems: 'center', justifyContent: 'center' },
